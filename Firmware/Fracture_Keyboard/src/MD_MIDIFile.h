...
typedef fs::FS SDFAT;     ///< Arduino filesystem class used by Fracture
typedef fs::File SDDIR;   ///< Arduino directory handle
typedef fs::File SDFILE;  ///< Arduino file handle
...
typedef struct
{
  ...
  uint32_t tick;    ///< absolute tick within this track (Fracture extension)
...
protected:
  ...
  uint32_t  _absoluteTicks; ///< absolute event time (Fracture extension)
...
public:
   ...
   * \return No return data.
   */
  void setFileFolder(const char* apath) { (void)apath; } // Full FS paths are used by Fracture.
...
protected:
  ...
  SDFAT     *_sd;                ///< filesystem descriptor supplied by calling program
  SDFILE    _fd;                ///< filesystem file descriptor
